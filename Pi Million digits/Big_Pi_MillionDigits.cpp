...
cout << "Program is initializing all the variables" <<endl<<endl;

    steady_clock::time_point start_time = steady_clock::now();
    steady_clock::time_point now_time;
    long elapsed_time;
    long cube_elapsed_time;
    long cube_elapsed_time_1;
    long cube_elapsed_time_2;
    long cube_elapsed_time_total;
    double percent_time;
    double percent_time_total;
steady_clock::time_point cube_time_start;
steady_clock::time_point cube_time_stop;
...
//Calculating previous values of a,r and s
		...
		cube_time_start = steady_clock::now();     // starting timer for cube root calculation 
              cube_root(s, one_minus_rrr);
		cube_time_stop = steady_clock::now();      // stopping timer for cube root calculation 
		cube_elapsed_time_total = duration_cast<milliseconds>(cube_time_stop - cube_time_start).count();  // Diffreence between start and stop time = time spent in cube root calculation
...
//Iteration steps starts from here
    ...
    cout << "Iterations will begin now" << endl;
    for(int j=1; j<=MAX_ITERATIONS; j++)
    {
     steady_clock::time_point iter_time = steady_clock::now();

    //COMPUTING tn+1;
    cout << "   " << "Time taken for iteration number " << j << " is ";cout.flush();
	...
	cube_time_stop = steady_clock::now();     // stopping timer for cube root calculation
	cube_elapsed_time_1 = duration_cast<milliseconds>(cube_time_stop - cube_time_start).count();  // Diffreence between start and stop time = time spent in cube root calculation
	...
	cube_time_start = steady_clock::now();
    cube_root(r_n_plus_one,one_minus_s_n_plus_one_cub);
	cube_time_stop = steady_clock::now();
	cube_elapsed_time_2 = duration_cast<milliseconds>(cube_time_stop - cube_time_start).count();
    ...
    mpf_set(r,r_n_plus_one);    // updating value of r to rn+1
	 ...
	 now_time = steady_clock::now();
         ...
         cout << " (" << elapsed_time << " msec)";

	cube_elapsed_time = cube_elapsed_time_1 + cube_elapsed_time_2;
	cube_elapsed_time_total = cube_elapsed_time_total + cube_elapsed_time;
	cout <<"  "<< "and  Time spent in computing cuberoot is"<< " (" << cube_elapsed_time << " msec)";

	percent_time = (double)cube_elapsed_time/elapsed_time;
	percent_time = percent_time * 100;
	cout << " " << percent_time<< " %" << endl;

    }

    cout << endl<< "Output of pi after inverting";
    mpf_div(pi, one,a);  // pi = 1/a

    now_time = steady_clock::now();
    duration<double>  total_time = duration_cast<duration<double>>(now_time - start_time);      // total time for million digits computation

    print(pi);           // calling a print function to print all the values of pi
    cout << "Total time taken for computing Pi upto million = " << total_time.count() << " seconds" << endl;
    cube_elapsed_time_total = cube_elapsed_time_total/1000;                                    // total time for calculating only cube root
    cout << "  Total time taken for computing only cube root = " << cube_elapsed_time_total << " seconds ";

    percent_time_total = (double)cube_elapsed_time_total/total_time.count();
    percent_time_total = percent_time_total * 100;                                             // % of time spent in calculating cube root
	cout << " (" << percent_time_total << " %)" << endl ;
    return 0;
}
...
void cube_root(mpf_t& x, const mpf_t a)
{
	...
	do
	{
		...
		mpf_mul(xxx,x,x) ;                   // x^2 = x * xd
		mpf_mul(xxx,x,xxx);                  // x^3 = x^2 * x
		...
		mpf_mul(x,x,temp);              // xnext has the result of cube root

		n++;

	}
	while((mpf_cmp(x,x_prev)!=0) && (n<15));
}
